...
from pde.callback import get_callbacks
from pde.utils import generate_id, instantiate_from_config, parse_config
...
DEFAULT_ENVIRONMENT = "./configs/local.yaml"
DEFAULT_CFG_NAME = "project_config.yaml"


def _has_wandb_credentials() -> bool:
    """Best-effort check for W&B credentials.

    W&B typically authenticates via the WANDB_API_KEY environment variable
    or via an entry in ~/.netrc created by `wandb login`.
    """
    ...
    if os.environ.get("WANDB_API_KEY"):
        return True
    try:
        return Path.home().joinpath(".netrc").exists()
    except Exception:
        return False
...
def get_config(opt, unknown):
    ...
    if logdir.exists():
        rank_zero_info(
            f"Run directory {logdir} already exists. Trying to continue run..."
        )
    ...
    # Prevent startup failures on machines without W&B credentials.
    if logger_state == "online" and not _has_wandb_credentials():
        rank_zero_warn(
            "W&B credentials not found (no WANDB_API_KEY and no ~/.netrc). "
            "Falling back to offline logging. Set WANDB_API_KEY or run `wandb login` to enable online sync."
        )
        logger_state = "offline"
...
def load_environment(env):
    ...
    # Make environment paths robust to Lightning/DDP subprocess working directories.
    # Treat relative paths as relative to the current working directory at launch.
    try:
        base_dir = Path.cwd()
        if "paths" in env_config and "PBDL_index" in env_config.paths:
            for key, value in env_config.paths.PBDL_index.items():
                if isinstance(value, str) and value:
                    env_config.paths.PBDL_index[key] = (
                        str((base_dir / value).resolve())
...
def get_loggers(config: DictConfig):
    ...
    wandb_logger = WandbLogger(
        ...
        entity=config.get("entity", None),  # Optional: log to a specific team
...
def set_initial_learning_rate(
    trainer, cpu, batch_size, base_learning_rate, trainer_config
):
    if not cpu:
...
def main_setup(config):
    ...
    try:
        ...
        acc_batches = trainer_config["params"].get("accumulate_grad_batches", 1)
        if "accumulate_grad_batches" not in model_config:
            model_config["accumulate_grad_batches"] = acc_batches

        model: lightning.LightningModule = instantiate_from_config(model_config)
        if config.get("compile", False):
            model.model = torch.compile(model.model)

        if getattr(model, "optimizer", "") == "muon":
            if "accumulate_grad_batches" in trainer_config["params"]:
                rank_zero_info(
                    f"Muon detected: Handing gradient accumulation ({acc_batches}) manually."
                )
                del trainer_config["params"]["accumulate_grad_batches"]

            # Double check model has the attribute set
            model.accumulate_grad_batches = acc_batches
        ...
        # Avoid launching DDP when running on a single device.
        # With `torch.compile`, single-device DDP can route through TorchDynamo's
        # distributed compilation backend and hit graph capture edge cases.
        try:
            devices = trainer_config.get("params", {}).get("devices", None)
            strategy = trainer_config.get("params", {}).get("strategy", None)
            if (
                (devices == 1 or devices == "1")
                and isinstance(strategy, str)
                and strategy.startswith("ddp")
            ):
                rank_zero_warn(
                    f"Overriding strategy='{strategy}' to 'auto' because devices={devices}. "
                    "DDP is unnecessary on a single device and can break torch.compile."
                )
                trainer_config["params"]["strategy"] = "auto"
        except Exception:
            pass
        ...
        if threading.current_thread() is threading.main_thread():
            ...
            def melk_fn(*args, **kwargs):
                pass

        # run
        if config.train:
            try:
                main_train(data, model, trainer, trainer_config)
            except Exception:
                melk_fn()
                raise
        ...
        cleanup(
            trainer, config.runtime.debug, config.runtime.resume, config.runtime.logdir
        )
...
def main_train(
    ...
    learning_rate = set_initial_learning_rate(
        trainer,
        cpu,
    ...
    model.muon_learning_rate = config.get("muon_learning_rate", 1e-03)
