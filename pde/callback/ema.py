...
class EMA(Callback):
    ...
    def __init__(
        self,
        decay: float,
        validate_original_weights: bool = True,
        every_n_steps: int = 1,
...
def ema_update(ema_model_tuple, current_model_tuple, decay):
    # Guard against edge cases where the optimizer has no parameters or EMA state
    # wasn't initialized/restored correctly. torch._foreach_* requires non-empty lists.
    if not ema_model_tuple:
        warning(
            "EMA update called with empty parameter list. This indicates that the optimizer has no parameters "
            "or that the EMA state wasn't initialized/restored correctly."
        )
        return
