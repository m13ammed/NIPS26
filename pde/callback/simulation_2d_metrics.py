...
from pde.callback.ema import EMAOptimizer
from pde.utils import get_pipeline, get_metrics
...
class Simulation2DMetricLogger(Callback):
    ...
    def update_metrics_impl(self, pipelines, trainer, pl_module, dataloader):
        ...
        for pipeline in pipelines.keys():
            metrics_torch[pipeline] = nn.ModuleList(get_metrics(self.metric_config)).to(
                pl_module.device
            )

        generator = torch.Generator(device=pl_module.device).manual_seed(
            trainer.global_rank
        )
        ...
        for pipeline in pipelines.keys():
            for metric in metrics_torch[pipeline]:
                ...
                if trainer.global_rank == 0:
                    ...
                    metric.save(
                        logdir + "/metrics/",
                        f"{pipeline}_{metric_name}_{trainer.current_epoch}.csv",
                    )
    ...
    def update_metrics(
        ...
        if use_ema and len(optimizers_ema) > 0:
            ...
            with ema.swap_ema_weights():
                pipelines = {
                    key: get_pipeline(value, pl_module)
    ...
    def check_frequency(self, check_idx):
        ...
        return ((check_idx + 1) % self.frequency) == 0 or (
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if (
            ...
            and self.check_frequency(pl_module.current_epoch)
        ):
...
class Simulation2DMetricLoggerCustom(Callback):
    ...
    def update_metrics_impl(self, trainer, pl_module, dataloader):
        ...
        metrics_torch = nn.ModuleList(get_metrics(self.metric_config)).to(
            pl_module.device
        )
        ...
        for batch in dataloader:
            class_labels = (
                batch["loading_metadata"]["dataset_idx"].flatten().to(pl_module.device)
            )

            prediction, target = pl_module.predict(
                batch,
                pl_module.device,
            ...
            for metric in metrics_torch:
                metric.update(
                    prediction[:, self.trim_start :],
                    target[:, self.trim_start :],
                    class_labels=class_labels,
                )

        # Aggregate raw metric states across all DDP ranks so computed values
        # and scalar summaries reflect the full test set.
        if dist.is_available() and dist.is_initialized():
            for metric in metrics_torch:
                if hasattr(metric, "metric_sum") and hasattr(metric, "total"):
                    dist.all_reduce(metric.metric_sum, op=dist.ReduceOp.SUM)
                    dist.all_reduce(metric.total, op=dist.ReduceOp.SUM)
        ...
        for metric in metrics_torch:
            ...
            if isinstance(metric_value, tuple):
                ...
                # Log scalar summaries at key timesteps (mean over datasets) instead
                # of the full tensor, which WandB renders as a histogram.
                for t_target in [0, 9]:
                    try:
                        t_idx = metric.times.index(t_target)
                        vals = metric_value[:, t_idx].float()  # (num_tasks,)

                        # Prefer counting only datasets that actually received
                        # updates at this timestep; fall back to finite values.
                        if hasattr(metric, "total"):
                            mask = metric.total[:, t_idx] > 0
                        else:
                            mask = torch.isfinite(vals)

                        valid_vals = vals[mask]
                        if valid_vals.numel() > 0:
                            metric_dict[f"{metric_name}_t{t_target}"] = (
                                valid_vals.mean().item()
                            )
                            metric_dict[f"{metric_name}_t{t_target}_n"] = int(
                                valid_vals.numel()
                            )
            ...
            if trainer.global_rank == 0:
                ...
                metric.save(
                    logdir + "/metrics/", f"{metric_name}_{trainer.current_epoch}.csv"
                )
