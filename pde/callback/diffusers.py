...
from pde.utils import get_pipeline
...
class Diffusers(Callback):
    ...
    def __init__(
        self,
        pipeline: Union[DictConfig, OmegaConf],
        frequency: int,
        repo_id: str,
        save_dir: str,
        ...
        if push_to_hub and "HUGGING_FACE_API_KEY" not in os.environ:
            print(
                "Please set the HUGGING_FACE_API_KEY environment variable to push to the hub."
            )
            push_to_hub = False
            self.token = ""
        else:
