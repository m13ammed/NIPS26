...
from pde.utils import instantiate_from_config, get_pipeline
...
def save_video(
    img, full_simulation, dataset_name, folder, savename, gen_video_frame_fn
):

    if shutil.which("ffmpeg") is None:
        rank_zero_warn(
            "ffmpeg not found on PATH; skipping video export. "
            "Install ffmpeg to enable mp4 logging."
        )
        return
...
class VideoLogger(Callback):
    def __init__(
        self,
        frequency: int,
        pipelines: Union[DictConfig, OmegaConf],
        max_videos: int = 4,
        num_frames: int = 10,
        clamp=True,
    ...
    def log_video(self, trainer, pl_module):
        ...
        pipelines = {
            key: get_pipeline(value, pl_module) for key, value in self.pipelines.items()
        }
        ...
        for pipeline in pipelines.keys():
            ...
            for c, video in enumerate(videos):
                savedir = logdir + "/videos"
                savename = f"pipeline-{pipeline}_e{current_epoch}_i{c}"
                save_video(
                    video,
                    full_simulation=video,
                    dataset_name=str(pipeline),
                    folder=savedir,
                    savename=savename,
                    gen_video_frame_fn=gen_video_frame_2d,
                )
            ...
            trainer.logger.experiment.log(
                {
                    pipeline: [
                        wandb.Video(
                            logdir
                            + f"/videos/pipeline-{pipeline}_e{current_epoch}_i{i}.mp4",
                            format="mp4",
                        )
    ...
    def check_frequency(self, check_idx):
        if (
            ((check_idx + 1) % self.frequency) == 0
            or (check_idx in self.log_steps)
            or (check_idx == 0 and self.log_first_step)
        ):
            try:
    ...
    def log_dataset(self, trainer: Trainer, pl_module: LightningModule):
        ...
        images = [
            self.prepare_plots(im).detach().cpu()
            for im in images_list[: self.max_images]
        ]
    ...
    def log_video(self, trainer, pl_module):
        ...
        for pipeline in pipelines.keys():
            ...
            for d in range(len(subsets)):
                ...
                full_simulation_list.append(
                    torch.concatenate([frames, target[0]], dim=0)
                )
                ...
                class_labels = torch.tensor(
                    class_labels, device=pl_module.device, dtype=torch.long
                )
                ...
                for c, video in enumerate(videos):
                    ...
                    trainer.logger.experiment.log(
                        {
                            f"{names_list[c]}_{pipeline}": wandb.Video(
                                savename,
                                format="mp4",
                            )
                        }
    ...
    def log_video(self, trainer, pl_module):
        ...
        try:
            ...
            for d in range(len(subsets)):
                ...
                video, reference = pl_module.predict(
                    batch,
                    pl_module.device,
                    num_frames=self.num_frames,
                    ...
                    trainer.logger.experiment.log(
                        {
                            ...
                            f"{simulation_name}_video": wandb.Video(
                                savename_video, format="mp4"
                            ),
                        }
                    )
                else:
            ...
            # Video logging should never crash training/testing.
