...
class Model(nn.Module):
    ## Factformer
    def __init__(self, args, window_size=4):
        ...
        ## Resolution
        self.padding = [(window_size - size % window_size) % window_size for size in args.shapelist]
        self.augmented_resolution = [(self.padding[i] + args.shapelist[i]) for i in range(len(self.padding))]
        ## models
        self.blocks = nn.ModuleList(
            [BasicLayer(dim=args.n_hidden, input_resolution=self.augmented_resolution, depth=2, num_heads=args.n_heads,
    ...
    def structured_geo(self, x, fx, T=None):
        ...
        fx = fx.permute(0, 2, 1).reshape(B, self.args.n_hidden, *self.augmented_resolution)
        if not all(item == 0 for item in self.padding):
            if len(self.args.shapelist) == 2:
