...
class DownsampleV2(nn.Module):
    def __init__(self, n_feat):
        ...
        self.body = nn.Sequential(
            nn.Conv2d(
                n_feat, n_feat // 4, kernel_size=3, stride=1, padding=1, bias=False
            ),
            nn.PixelUnshuffle(2),
        )
...
class WindowAttention2DTime(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            attn = attn.view(B // mask_shape, mask_shape, self.num_heads, N, N)
            attn = attn + attn_mask.unsqueeze(1).unsqueeze(0).to(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(1, self.num_encoder_layers):
            ...
            self.__setattr__(
                f"up{i + 1}_{i}", Upsample(hidden_size_upsample, keep_dim=keep_dim)
            )
            self.__setattr__(
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.__name__ = "PDE_Transformer"
        self.args = args

        if args.geotype != "structured_2D":
            raise ValueError(
                f"Unsupported geotype {args.geotype}. Only structured_2D is supported."
            )
    ...
    def forward(self, x, fx=None, T=None, geo=None):
        ...
        out = self.pde_model(inp, T_input, None)
