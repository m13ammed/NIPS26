...
from layers.MWT_Layers import MWT_CZ1d, MWT_CZ2d, MWT_CZ3d
    ...
    def __init__(self, args, alpha=2, L=0, c=1, base='legendre', s1=128, s2=128):
        ...
        if args.unified_pos and args.geotype != 'unstructured':  # only for structured mesh
            ...
            self.preprocess = MLP(args.fun_dim + args.ref ** len(args.shapelist), args.n_hidden * 2,
                                  self.WMT_dim, n_layers=0, res=False, act=args.act)
        else:
        ...
        if self.args.geotype == 'unstructured':
            self.fftproject_in = SpectralConv2d_IrregularGeo(self.WMT_dim, self.WMT_dim, args.modes, args.modes, s1, s2)
            self.fftproject_out = SpectralConv2d_IrregularGeo(self.WMT_dim, self.WMT_dim, args.modes, args.modes, s1,
                                                              s2)
            self.iphi = IPHI()
    ...
    def structured_geo(self, x, fx, T=None):
        ...
        for i in range(self.args.n_layers):
            ...
            if i < self.args.n_layers - 1:
                x = F.gelu(x)
        x = x.reshape(B, -1, self.WMT_dim).permute(0, 2, 1).contiguous() \
