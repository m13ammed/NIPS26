...
from data.torch.dataset import Dataset as PBDLDataset
from data.variable_dt_dataset import VariableDtDataset
from data.metadata_remapping import convert_pde, convert_fields, convert_constants
from data.metadata_remapping import (
    convert_domain_extent,
    convert_dt,
    convert_reynolds_number,
    convert_boundary_conditions,
)
...
def unpack_pbdl_data_format(pbdl_data: tuple, variable_dt_format: bool = False) -> list:
    ...
    time_step_stride = (
        torch.tensor(time_step_stride) if variable_dt_format else torch.tensor([1])
    )
...
class MetadataDataset(Dataset):
    ...
    def __init__(
        self,
        dataset: PBDLDataset | VariableDtDataset | Subset,
        loading_metadata: dict,
        transforms=None,
    ):
        super().__init__()
        ...
        if hasattr(dataset, "sel_channels"):
            if dataset.sel_channels is not None:
                metadata["Fields"] = [
                    metadata["Fields"][i] for i in dataset.sel_channels
                ]
        if hasattr(dataset, "sel_constants"):
            if dataset.sel_constants is not None:
                metadata["Constants"] = [
                    metadata["Constants"][i] for i in dataset.sel_constants
                ]
        ...
        if isinstance(metadata["Resolution"], list):
            ...
            raise ValueError(
                "Resolution %s must be a list or an integer but has type %s"
                % (metadata["Resolution"], type(metadata["Resolution"]))
            )
        ...
        if not "Domain Extent" in metadata:
            ...
            raise ValueError(
                "Domain Extent %s must be a list or a number but has type %s"
                % (metadata["Domain Extent"], type(metadata["Domain Extent"]))
            )
        ...
        self.physical_metadata = {
            ...
            "Boundary Conditions": convert_boundary_conditions(
                metadata["Boundary Conditions"], metadata["Boundary Conditions Order"]
            ),
    ...
    def __getitem__(self, index):
        ...
        data, constants_norm, constants, time_step_stride = unpack_pbdl_data_format(
            from_loader, variable_dt_format
        )
        ...
        return {
            ...
            "physical_metadata": copy.deepcopy(
                self.physical_metadata
            ),  # copy in case models or other dataset make changes to these tensors
            "loading_metadata": copy.deepcopy(
                self.loading_metadata
            ),  # copy in case models or other dataset make changes to these tensors
        }
