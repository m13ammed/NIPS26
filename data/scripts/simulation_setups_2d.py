...
def initial_condition_generator(p: dict, varied: dict) -> ex.ic.BaseRandomICGenerator:
    ...
    if init_type == "Truncated Fourier":
        cutoff = np.random.randint(
            p["Initial Frequency Cutoff (min)"], p["Initial Frequency Cutoff (max)"]
        )
        ...
        powerlaw = np.random.uniform(
            p["Initial Powerlaw Exponent (min)"], p["Initial Powerlaw Exponent (max)"]
        )
        ...
        intensity = np.random.uniform(
            p["Initial Noise Intensity (min)"], p["Initial Noise Intensity (max)"]
        )
...
def get_advection(
    is_test_set: bool, seed: int
) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    p = {
        "PDE": "Advection",
        "Dimension": 2,
        "Fields Scheme": "d",
    ...
    stepper = ex.stepper.Advection(
        2, p["Domain Extent"], p["Resolution"], p["Dt"] / p["Sub Steps"], velocity=vel
    )
...
def get_diffusion(
    ...
    stepper = ex.stepper.Diffusion(
        2, p["Domain Extent"], p["Resolution"], p["Dt"] / p["Sub Steps"], diffusivity=nu
    )
...
def get_dispersion(
    ...
    dispersivity = np.random.uniform(
        p["Dispersivity (min)"], p["Dispersivity (max)"], 2
    )
...
def get_hyper_diffusion(
    ...
    diffusivity = np.random.uniform(
        p["Hyper-Diffusivity (min)"], p["Hyper-Diffusivity (max)"]
    )
...
def get_korteweg_de_vries(
    ...
    stepper = ex.stepper.KortewegDeVries(
        2, extent, p["Resolution"], p["Dt"] / p["Sub Steps"], diffusivity=nu
    )
...
def get_kuramoto_sivashinsky(
    ...
    stepper = ex.stepper.KuramotoSivashinsky(
        2, extent, p["Resolution"], p["Dt"] / p["Sub Steps"]
    )
...
def get_fisher_kpp(
    ...
    ic_gen = ex.ic.ClampingICGenerator(
        initial_condition_generator(p, varied), limits=(0, 1)
    )
...
def get_gray_scott(
    is_test_set: bool, seed: int, variant: str
) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    ...
    ic_gen = ex.ic.RandomMultiChannelICGenerator(
        [
            ex.ic.RandomGaussianBlobs(
                2,
                domain_extent=p["Domain Extent"],
                position_range=(0.4, 0.6) if variant in ["kappa"] else (0.2, 0.8),
                num_blobs=4,
                one_complement=True,
            ),
            ex.ic.RandomGaussianBlobs(
    ...
    stepper = ex.stepper.reaction.GrayScott(
        2,
        p["Domain Extent"],
        p["Resolution"],
        p["Dt"] / p["Sub Steps"],
        diffusivity_1=p["Diffusivity A"],
        diffusivity_2=p["Diffusivity B"],
        feed_rate=feed,
        kill_rate=kill,
    )
