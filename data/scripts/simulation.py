...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_set", default=False, action="store_true"
    )  # NOTE: omit argument to set to false
    parser.add_argument("--out_name", type=str, default="kolm_flow")
    parser.add_argument(
        "--out_path",
        type=str,
        default="/mnt/data2/pbdl-datasets-local/2d_apebench_256_large",
    )
    parser.add_argument("--num_sims", type=int, default=60)
    parser.add_argument(
        "--render_only", default=False, action="store_true"
    )  # NOTE: omit argument to set to false
    parser.add_argument("--gpu_id", type=str, default="0")
    parser.add_argument(
        "--low-res", default=False, action="store_true"
    )  # NOTE: omit argument to set to false
...
def generate_data(
    ...
    while data_id < num_sims:
        ...
        if dimension == 2:
            p_fixed, p_varying, stepper, u_next = get_setup_2d(
                sim_type, is_test_set, sim_id
            )
        else:
            p_fixed, p_varying, stepper, u_next = get_setup_3d(
                sim_type, is_test_set, sim_id
            )
        ...
        repeated_stepper = ex.RepeatedStepper(
            stepper, p_fixed["Sub Steps"]
        )  # omit writing substeps
        ...
        for i in range(time_steps):
            ...
            if i % 10 == 0 or i == time_steps - 1:
                print(
                    "\tSim %d/%d, Time step %d, mean: %s (%s remaining)"
                    % (data_id + 1, num_sims, i, jnp.mean(u_next), remStr)
                )
        ...
        if not render_only:
            with h5py.File(out_dir + ".hdf5", "a") as h5py_file:
                ...
                if low_res:
                    ...
                    if len(data.shape) == 4:
                        data = data.reshape(
                            data.shape[0], data.shape[1], 256, 8, 256, 8
                        ).mean(axis=(3, 5))
                    elif len(data.shape) == 3:
                        data = data.reshape(data.shape[0], 256, 8, 256, 8).mean(
                            axis=(2, 4)
                        )
                    else:
