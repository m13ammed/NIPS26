...
def get_long_to_short_pde_map(index_path=None):
    """
    Returns a dict mapping long PDE names (lowercase) to short dataset keys.
    index_path: path to global_index.json (default: relative to this file)
    """
    if index_path is None:
        index_path = os.path.join(
            os.path.dirname(__file__), "pbdl", "global_index.json"
        )
    if not os.path.isfile(index_path):
        raise FileNotFoundError(f"global_index.json not found at {index_path}")
    with open(index_path, "r") as f:
        index = json.load(f)
    long_to_short = {}
    for short_name, meta in index.items():
        pde_name = meta.get("PDE", None)
        if pde_name:
            long_to_short[pde_name.lower()] = short_name
...
def normalize_positive_value(x: float) -> float:
    ...
    return x / (
        1 + x
    )  # equivalent to 1 / (1 + (1/x)) or 1 / (1 + e^(-ln(x))), i.e. sigmoid of ln(x)
...
def convert_boundary_conditions(
    boundary_conditions: list[str], boundary_condition_order: list[str]
) -> torch.Tensor:
    """
    ...
    assert len(boundary_conditions) == len(
        boundary_condition_order
    ), "Boundary conditions and order must have the same length"

    result = torch.zeros(len(boundary_conditions)).float()

    for i in range(len(boundary_conditions)):
        result = update_boundary_condition(
            result,
            boundary_conditions[i],
            boundary_condition_order[i],
            update_opposite=False,
        )
...
def update_boundary_condition(
    ...
    boundary_condition_direction: str,
    update_opposite: bool = True,
) -> torch.Tensor:
    """
    ...
    if idx > tensor.shape[0]:
        raise ValueError(
            f"Boundary condition direction {boundary_condition_direction} not possible for tensor with shape {tensor.shape}"
        )
    ...
    opposite_boundary_condition_idx = target_order.index(
        opposite_direction[boundary_condition_direction]
    )
    opposite_boundary_condition = all_boundary_conditions[
        tensor[opposite_boundary_condition_idx].int().item()
    ]
    ...
    if boundary_condition == "periodic" and opposite_boundary_condition != "periodic":
        tensor[opposite_boundary_condition_idx] = all_boundary_conditions.index(
            "periodic"
        )
    return tensor
