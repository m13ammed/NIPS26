...
from data.types.acdm_2d import acdm_2d_datasets, acdm_2d_transforms
from data.types.ape_2d import ape_2d_datasets, ape_2d_transforms
from data.types.ape_2d_xxl import ape_2d_xxl_datasets, ape_2d_xxl_transforms
from data.types.well_2d import well_2d_datasets, well_2d_transforms
from data.utils import SubsetSequentialSampler


def get_transforms(
    datatype: str, dataset_name: str
...
class PBDLDataModule(lightning.pytorch.LightningDataModule):
    ...
    def __init__(
        ...
        self.variable_dt_stride_maximum = (
            variable_dt_stride_maximum if variable_dt else 1
        )
    ...
    def setup(self, stage: str):
        ...
        self._prepare_gpu_split_and_cache(
            "fit", MetadataDataset(set_train, loading_metadata_train, transform_train)
        )
        self._prepare_gpu_split_and_cache(
            "validate", MetadataDataset(set_val, loading_metadata_train, transform_val)
        )
        self._prepare_gpu_split_and_cache(
            "test", MetadataDataset(set_test, loading_metadata_test, transform_test)
        )
