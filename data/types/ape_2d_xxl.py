...
from torchvision.transforms.v2 import Transform, ToDtype, Compose
from data.pbdl.dataset import Dataset as PBDLDataset

from data.variable_dt_dataset import VariableDtDataset
...
def _available_sim_ids(dataset_directory: str, dataset_name: str) -> list[int]:
    """Return sorted available simulation ids (sim0, sim1, ...) in a local HDF5 file."""
    path = os.path.join(dataset_directory, f"{dataset_name}.hdf5")
    ...
    with h5py.File(path, "r") as f:
        ...
        for key in sims.keys():
            m = re.fullmatch(r"sim(\d+)", str(key))
            if m:
                ids.append(int(m.group(1)))
        return sorted(ids)


def _split_train_test_sim_ids(
    sim_ids: list[int], train_fraction: float
) -> tuple[list[int], list[int]]:
    if not sim_ids:
        return [], []
    n = len(sim_ids)
    # keep at least 1 train sample; keep at least 1 test sample when possible
    n_train = max(1, int(round(n * train_fraction)))
    if n > 1:
        n_train = min(n_train, n - 1)
    return sim_ids[:n_train], sim_ids[n_train:]


def ape_2d_xxl_datasets(
    dataset_name: str,
    ...
    **kwargs,
) -> tuple[PBDLDataset, PBDLDataset, PBDLDataset]:
    r"""
    ...
    # Make dataset paths robust to Lightning/DDP subprocess working directories.
    ...
    if dataset_name in [
        ...
        if test_variable_dt_stride_maximum <= 1:
            ...
            test = VariableDtDataset(
                **params_test, maximum_dt=test_variable_dt_stride_maximum, seed=seed
            )

    # joint training and test data file, split by simulation
    else:

        # Determine available sim ids.
        # - If we already have a local file, use its contents.
        # - Otherwise, trigger a download once (via a probe dataset) and re-check the local file.
        sim_ids = _available_sim_ids(dataset_directory, dataset_name)
        if not sim_ids:
            probe_params = {
                "dset_name": dataset_name,
                "local_datasets_dir": dataset_directory,
                "global_dataset_dir": dataset_directory,
                "time_steps": unrolling_steps,
                "intermediate_time_steps": intermediate_time_steps,
                "normalize_const": (
                    normalize_const if not "gs_" in dataset_name else None
                ),
                "normalize_data": normalize_data,
            }
            probe = PBDLDataset(**probe_params)
            try:
                probe.dset.close()
            except Exception:
                pass
            sim_ids = _available_sim_ids(dataset_directory, dataset_name)
            if not sim_ids:
                sim_ids = list(range(getattr(probe, "num_sims", 0)))

        if dataset_name in ["gs_delta", "gs_theta", "gs_iota", "gs_kappa"]:
            train_sims, test_sims = _split_train_test_sim_ids(
                sim_ids, train_fraction=0.8
            )
            ...
            train_sims, test_sims = _split_train_test_sim_ids(
                sim_ids, train_fraction=(50.0 / 60.0)
            )
        else:
        ...
        if not train_sims:
            raise FileNotFoundError(
                f"Could not determine simulations for dataset '{dataset_name}'. "
                f"Expected a local file at {os.path.join(dataset_directory, dataset_name + '.hdf5')}"
            )
