...
from data.metadata_dataset import MetadataDataset
from data.metadata_remapping import update_boundary_condition
from data.pbdl_module import get_datasets, get_transforms
from data.utils import SubsetSequentialSampler
...
class MultiDataModule(lightning.pytorch.LightningDataModule):
    ...
    def setup(self, stage: str):
        ...
        for i in range(len(self.dataset_names)):
            ...
            datatype = (
                self.dataset_type[i]
                if (
                    isinstance(self.dataset_type, ListConfig)
                    or isinstance(self.dataset_type, list)
                )
                else self.dataset_type
            )
            directory = self.path_index[datatype]
            steps = (
                self.unrolling_steps[i]
                if (
                    isinstance(self.unrolling_steps, ListConfig)
                    or isinstance(self.unrolling_steps, list)
                )
                else self.unrolling_steps
            )
            test_steps = (
                self.test_unrolling_steps[i]
                if (
                    isinstance(self.test_unrolling_steps, ListConfig)
                    or isinstance(self.test_unrolling_steps, list)
                )
                else self.test_unrolling_steps
            )
            variable_dt = (
                self.variable_dt[i]
                if isinstance(self.variable_dt, ListConfig)
                else self.variable_dt
            )
            test_variable_dt = (
                self.test_variable_dt[i]
                if isinstance(self.test_variable_dt, ListConfig)
                else self.test_variable_dt
            )
            variable_dt_stride = (
                self.variable_dt_stride_maximum[i]
                if isinstance(self.variable_dt_stride_maximum, ListConfig)
                else self.variable_dt_stride_maximum
            )
            variable_dt_stride = variable_dt_stride if variable_dt else 1

            test_variable_dt_stride = (
                self.test_variable_dt_stride_maximum[i]
                if isinstance(self.test_variable_dt_stride_maximum, ListConfig)
                else self.test_variable_dt_stride_maximum
            )
            test_variable_dt_stride = test_variable_dt_stride if test_variable_dt else 1

            # prepare transforms and datasets
            transform_train, transform_val, transform_test = get_transforms(
                datatype, name
            )
            set_train, set_val, set_test = get_datasets(
                datatype,
                name,
                directory,
                steps,
                ...
                test_variable_dt_stride,
                self.normalize_data,
                self.normalize_const,
                self.crop_size,
                **self.additional_kwargs,
            )

            # add metadata
            loading_metadata_train = {
                "type": datatype,
                "unrolling_steps": steps,
                "dataset_idx": i,
            }
            loading_metadata_test = {
                "type": datatype,
                "unrolling_steps": test_steps,
                "dataset_idx": i,
            }

            meta_train = MetadataDataset(
                set_train, loading_metadata_train, transform_train
            )
        ...
        self._prepare_gpu_split_and_cache(
            "fit",
            ConcatDatasetDifferentShapes(
                self.subsets_train,
    ...
    def _prepare_gpu_split_and_cache(
        self, stage: str, dataset: Dataset, after_cache_transforms: Transform = None
    ):
        r"""
        ...
        if stage == "fit" and not self.set_train:
            ...
            if self.cache_strategy in ["all"]:
                self.set_train = CachedDataset(
                    dataset, after_cache_transforms, self.max_cache_size
                )
    ...
    def val_dataloader(self):
        return DataLoader(
            ...
            num_workers=(
                0 if self.cache_strategy in ["testAndVal", "all"] else self.num_workers
            ),
            pin_memory=True,
        )
...
class ConcatDatasetDifferentShapes(ConcatDataset[MetadataDataset]):
    ...
    def __init__(
        self,
        datasets: Iterable[MetadataDataset],
        different_resolution_strategy: str,
        ...
        for dataset in datasets:
            if dataset.dimension != self.dimension:
                raise ValueError(
                    "Concatenating datasets with different dimensions is not supported!"
                )
            ...
            for i in range(dataset.dimension):
                self.min_resolution[i] = min(
                    self.min_resolution[i], dataset.resolution[i]
                )
                self.max_resolution[i] = max(
                    self.max_resolution[i], dataset.resolution[i]
                )
        ...
        if target_size is None:
            if self.different_resolution_strategy == "none":
                ...
                raise ValueError(
                    f"Unknown resolution strategy: {self.different_resolution_strategy}"
                )
    ...
    def process_sample(self, sample, add_batch_dim=False):
        ...
        if self.dimension == 2 and self.target_size is not None:
            ...
            if (
                data.shape[2] == self.target_size[0]
                and data.shape[3] == self.target_size[1]
            ):
                pass
            ...
            # data too small -> bilinear interpolation
                ...
                data = torch.nn.functional.interpolate(
                    data, size=self.target_size, mode="bilinear", align_corners=False
                )
            ...
            # data too large -> random crop
                ...
                start = (
                    torch.randint(
                        0, data.shape[2] - self.target_size[0] + 1, (1,)
                    ).item(),
                    torch.randint(
                        0, data.shape[3] - self.target_size[1] + 1, (1,)
                    ).item(),
                )
                end = (
                ...
                if start[0] > 0:
                    metadata["Boundary Conditions"] = update_boundary_condition(
                        metadata["Boundary Conditions"], "open", "x negative"
                    )
                if end[0] < data.shape[2]:
                    metadata["Boundary Conditions"] = update_boundary_condition(
                        metadata["Boundary Conditions"], "open", "x positive"
                    )
                if start[1] > 0:
                    metadata["Boundary Conditions"] = update_boundary_condition(
                        metadata["Boundary Conditions"], "open", "y negative"
                    )
                if end[1] < data.shape[3]:
                    metadata["Boundary Conditions"] = update_boundary_condition(
                        metadata["Boundary Conditions"], "open", "y positive"
                    )
        ...
        # 3D spatial adjustment
            ...
            if (
                data.shape[2] == self.target_size[0]
                and data.shape[3] == self.target_size[1]
                and data.shape[4] == self.target_size[2]
            ):
                pass
            ...
            # data too small -> trilinear interpolation
                ...
                data = torch.nn.functional.interpolate(
                    data,
                    size=(data.shape[1],) + self.target_size,
                    mode="trilinear",
                    align_corners=False,
                )
            ...
            # data too large -> random crop
                ...
                start = (
                    ...
                    torch.randint(
                        0, data.shape[4] - self.target_size[2] + 1, (1,)
                    ).item(),
                )
                end = (
                    start[0] + self.target_size[0],
                ...
                if start[2] > 0:
                    metadata["Boundary Conditions"] = update_boundary_condition(
                        metadata["Boundary Conditions"], "open", "z negative"
                    )
                if end[2] < data.shape[4]:
                    metadata["Boundary Conditions"] = update_boundary_condition(
                        metadata["Boundary Conditions"], "open", "z positive"
                    )
            ...
            raise ValueError(
                "Datasets with dimensionality %d are not supported." % self.dimension
            )
        ...
        if data.shape[1] == self.target_channels:
            ...
            pad = torch.zeros(
                data.shape[0], self.target_channels - data.shape[1], *data.shape[2:]
            )
            ...
            raise NotImplementedError(
                "Reducing the number of channels is currently not implemented."
            )
        ...
        if constants.shape[0] == self.target_constants:
            ...
            pad_constants = torch.zeros(
                self.target_constants - len(metadata["Constants"])
            )
            metadata["Constants"] = torch.cat([metadata["Constants"], pad_constants], 0)
        else:
            raise NotImplementedError(
                "Reducing the number of constants is currently not implemented."
            )
        ...
        loading_metadata["dataset_idx"] = torch.Tensor(
            [loading_metadata["dataset_idx"]]
        ).long()
        loading_metadata["unrolling_steps"] = torch.Tensor(
            [loading_metadata["unrolling_steps"]]
        ).long()
        ...
        if add_batch_dim:
            ...
            metadata["Boundary Conditions"] = metadata["Boundary Conditions"].unsqueeze(
                0
            )

            loading_metadata["dataset_idx"] = loading_metadata["dataset_idx"].unsqueeze(
                0
            )
            loading_metadata["unrolling_steps"] = loading_metadata[
                "unrolling_steps"
            ].unsqueeze(0)
...
def get_subdatasets_from_dataloader(
    ...
    while not isinstance(dataset, ConcatDatasetDifferentShapes):
        if isinstance(dataset, torch.utils.data.Subset) or isinstance(
