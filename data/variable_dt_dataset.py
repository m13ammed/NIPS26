...
class VariableDtDataset(PBDLDataset):
    ...
    def __init__(self, maximum_dt: int, seed: int = None, *args, **kwargs):
        ...
        assert (
            self.num_frames > self.time_steps * maximum_dt
    ...
    def __getitem__(self, idx):
        ...
        while (
            input_frame_idx + self.time_steps * time_step_stride
            >= self.num_frames - self.trim_end
        ):
            time_step_stride -= 1
        if time_step_stride < 1:
            raise ValueError(
                "This should not happen. Time step stride is too small for input frame {} and time steps {}.".format(
                    input_frame_idx, self.time_steps
                )
            )
