...
from data.pbdl import fetcher, logging
from data.pbdl import normalization as norm
from data.pbdl.logging import corrupt, fail, info, success, warn
from data.pbdl.utilities import (
    get_meta_data,
    get_sel_const_sim,
    get_sel_const_sim_v2,
    scan_local_dset_dir,
)

config = {
    "hf_repo_id": "thuerey-group/pde-transformer-ape2d",
    "local_datasets_dir": "./datasets_256/",
    "global_dataset_dir": "./datasets_256/",
    "dataset_ext": ".hdf5",
}
...
class Dataset:
    def __init__(
        ...
        **kwargs,
        ...
        self.norm_strat_data = (
            norm.get_norm_strat_from_str(
                normalize_data, self.dset, self.sel_const, const=False
            )
            if normalize_data
            else None
        )
        self.norm_strat_const = (
            norm.get_norm_strat_from_str(
                normalize_const, self.dset, self.sel_const, const=True
            )
            if normalize_const
            else None
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.crop_size is None:
            ...
            crop_dim_list = [
                self.rng.integers(low=0, high=dim - self.crop_size, size=1)[0]
                for dim in dim_list
            ]
        ...
        if self.intermediate_time_steps:
            if self.crop_size is None:
                ...
                if len(dim_list) == 2:
                    target = sim[
                        input_frame_idx + 1 : target_frame_idx + 1,
                        :,
                        crop_dim_list[0] : crop_dim_list[0] + self.crop_size,
                        crop_dim_list[1] : crop_dim_list[1] + self.crop_size,
                    ]
